...
export default function Cart() {
  ...
  const productsArray = useLocalStorage('cart')?.getItem();
    ...
    const fetchData = async () => {
      try {
        ...
        if (response.ok) {
          ...
          setData(responseData.res); // Updated to set the state directly with the fetched data
        } else {
...
// export const Cart:React.FC<Props>= ({productsFromServer})=> {
...
//     // na sztywno, dopoki nie bedzie przyciskow 
//     localCart={"1":1,"2":5};

//     const products:{product:Product,quantity:number}[] = [];

//     for (const [key, value] of Object.entries(localCart)) {
//         const product = productsFromServer.find(p=>p.id===Number(key));
